...
/*func kmeans(dist func(p1, p2 interface{})float64, updateClust func(int, []int)interface{}) func([]interface{}, int, int)([]interface{}, []int, error) {
...
func kmeans(entities []interface{}, k, maxIters int) ([]interface{}, []Cluster, error) {
   numEntities := len(entities)
   ...
   for i:=0; i<maxIters; i++ {
      ...
      for j:=0; j<numEntities; j++ {
         ...
         for l:=0; l<k; l++ {
            dist := euclideanDist(entities[j], centers[l])
            if dist < minDist {
      ...
      // compute new centers
      for j:=0; j<k; j++ {
         centers[j] = updateCluster(clusters[j])
      }
...
func main() {
   ...
   fmt.Println(kmeans(interfacePoints, 2, 10))
